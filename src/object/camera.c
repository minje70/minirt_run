...
{
	if (s_set->amb_light != -1 || s_set->color.x != -1 || s_set->color.y != -1 || s_set->color.z != -1)
	{
		perror("A는 한번만 적용할 수 있습니다.");
		exit(1);
	}
	s_set->color = color;
...
int set_camera(t_camera	*cam)
{
	...
	cam->left_top_corner = vplus(vplus(vplus(cam->w, cam->origin), vdivide(cam->horizontal, 2)), vdivide(cam->vertical, 2));
	return (1);
}
